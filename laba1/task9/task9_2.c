...
int main() {
    ...
    int *first_array = malloc(size_a * sizeof(int));
    int *second_array = malloc(size_b * sizeof(int));

    if (first_array == NULL || second_array == NULL) {
        printf("Problems with malloc\n");
        if (first_array != NULL) free(first_array);
        if (second_array != NULL) free(second_array);
        return MEMORY_ERROR;
    }
    ...
    int *last_array = malloc(size_a * sizeof(int));
    if (last_array == NULL) {
        printf("Problems with malloc\n");
        if (first_array != NULL) free(first_array);
        if (second_array != NULL) free(second_array);
